...
public class ZhaoGongZuoDAO  implements DataDAOImp{
	public boolean Insert(Object con1){
		ZhaoGongZuo con=(ZhaoGongZuo)con1;
		boolean b=true;
	...
	public List SelectAll(int first,int end){
		...
		try{
			session=HibernateSessionFactory.getSession();	
			query=session.createQuery("from ZhaoGongZuo Order by  time desc");
	...
	public List SelectWhere(Object con1,int first,int end){
		...
		try{
			session=HibernateSessionFactory.getSession();
			Criteria criteria=session.createCriteria(ZhaoGongZuo.class);
			if(con.getId()!=0){
			query=session.createQuery("from ZhaoGongZuo where id="+con.getId());
			list=query.list();
			}else
