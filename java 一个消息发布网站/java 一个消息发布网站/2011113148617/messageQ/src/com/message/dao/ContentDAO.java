...
import com.message.factory.*;
import com.message.bean.Content;

public class ContentDAO implements DataDAOImp{
	public boolean Insert(Object con1){
		Content con=(Content)con1;
		boolean b=true;
	...
	public List SelectAll(int first,int end){
		...
		try{
			session=HibernateSessionFactory.getSession();	
			query=session.createQuery("from Content Order by  time desc");
	...
	public List SelectWhere(Object con1,int first,int end){
		...
		try{
			...
			if(con.getId()!=0){
			query=session.createQuery("from Content where id="+con.getId());
			list=query.list();
			}else
