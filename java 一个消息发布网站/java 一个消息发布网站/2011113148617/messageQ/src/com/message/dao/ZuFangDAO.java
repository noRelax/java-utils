...
public class ZuFangDAO implements DataDAOImp{
	public boolean Insert(Object con1){
		ZuFang con=(ZuFang)con1;
		boolean b=true;
	...
	public List SelectAll(int first,int end){
		...
		try{
			session=HibernateSessionFactory.getSession();	
			query=session.createQuery("from ZuFang Order by  time desc");
	...
	public List SelectWhere(Object con1,int first,int end){
		...
		try{
			session=HibernateSessionFactory.getSession();
			Criteria criteria=session.createCriteria(ZuFang.class);
			if(con.getId()!=0){
			query=session.createQuery("from ZuFang where id="+con.getId());
			list=query.list();
			}else
