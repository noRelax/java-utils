...
public class ErShouDAO implements DataDAOImp{
	public boolean Insert(Object con1){
		ErShou con=(ErShou)con1;
		boolean b=true;
	...
	public List SelectAll(int first,int end){
		...
		try{
			session=HibernateSessionFactory.getSession();	
			query=session.createQuery("from ErShou Order by  time desc");
	...
	public List SelectWhere(Object con1,int first,int end){
		...
		try{
			session=HibernateSessionFactory.getSession();
			Criteria criteria=session.createCriteria(ErShou.class);
			if(con.getId()!=0){
			query=session.createQuery("from ErShou where id="+con.getId());
			list=query.list();
			}else
