...
查看：
<%
	List chushou=(List)request.getAttribute("zhaopin");
...
<%  while(chushouI.hasNext()){ 
	Content con4=(Content)chushouI.next();
%>
<tr>
<td><%=con4.getId() %></td>
<td><a href="ContentServlet?id=<%=con4.getId() %>"><%=con4.getTitle()+" 发布日期"+con4.getTime() %></a></td>
</tr>
<%} %>
</table>
<a href="ContentProServlet?nowpage=<%=nowpage+1 %>">下一页</a>
<% 
if(nowpage<=1)
{
%>
<a href="ContentProServlet?nowpage=1">上一页</a>
<%
}else{
%>
<a href="ContentProServlet?nowpage=<%=nowpage-1 %>">上一页</a>
<% }%>
</div>
</div>
