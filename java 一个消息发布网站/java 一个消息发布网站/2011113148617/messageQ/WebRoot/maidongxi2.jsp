...
<td width="484">标题</td>
 ...
 <%  while(chushouI.hasNext()){ 
	ErShou con4=(ErShou)chushouI.next();
%>
<tr>
<td><%=con4.getId() %></td>
<td><a href="ErShouServlet?id=<%=con4.getId() %>"><%=con4.getTitle()+" 发布日期"+con4.getTime()%></a></td>
</tr>
<%} %>
</table>
<a href="ErShouProServlet?nowpage=<%=nowpage+1 %>">下一页</a>
<% 
if(nowpage<=1)
{
%>
<a href="ErShouProServlet?nowpage=1">上一页</a>
<%
}else{
%>
<a href="ErShouProServlet?nowpage=<%=nowpage-1 %>">上一页</a>
<% }%>
</div>
