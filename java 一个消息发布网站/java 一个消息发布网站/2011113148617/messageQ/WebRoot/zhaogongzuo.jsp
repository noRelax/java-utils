...
<td width="484">标题</td>
 ...
 <%  while(chushouI.hasNext()){ 
	ZhaoGongZuo con4=(ZhaoGongZuo)chushouI.next();
%>
<tr>
<td><%=con4.getId() %></td>
<td><a href="ZhaoGongZuoServlet?id=<%=con4.getId() %>"><%=con4.getTitle()+" 发布日期"+con4.getTime() %></a></td>
</tr>
<%} %>
</table>
<a href="ZhaoGongZuoProServlet?nowpage=<%=nowpage+1 %>">下一页</a>
<% 
if(nowpage<=1)
{
%>
<a href="ZhaoGongZuoProServlet?nowpage=1">上一页</a>
<%
}else{
%>
<a href="ZhaoGongZuoProServlet?nowpage=<%=nowpage-1 %>">上一页</a>
<% }%>
</div>
</div>
