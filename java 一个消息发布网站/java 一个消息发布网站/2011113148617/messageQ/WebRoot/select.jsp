...
<%
	String keyword=(String)request.getAttribute("keywrod2");
	List zhaopin=(List)request.getAttribute("zhaopin");
	List yingpin=(List)request.getAttribute("yingpin");
	List qiougou=(List)request.getAttribute("qiougou");
	List chushou=(List)request.getAttribute("chushou");
	List zufang=(List)request.getAttribute("zufang");
	int nowpage=(Integer)request.getAttribute("nowpage");
	Iterator zhaopinI=zhaopin.iterator();
	Iterator yingpinI=yingpin.iterator();
	Iterator qiougouI=qiougou.iterator();
	Iterator chushouI=chushou.iterator();
	Iterator zufangI=zufang.iterator();
 %>
查看招聘：
...
<%	while(zhaopinI.hasNext()){ 
	Content con=(Content)zhaopinI.next();
%>
<tr>
<td><%=con.getId() %></td>
<td><a href="ContentServlet?id=<%=con.getId() %>"><%=con.getTitle() +" 发布日期"+con.getTime()%></a></td>
</tr>
<%} %>
</table>
查看应聘：
...
<%	while(yingpinI.hasNext()){ 
	ZhaoGongZuo con2=(ZhaoGongZuo)yingpinI.next();
%>
<tr>
<td><%=con2.getId() %></td>
<td><a href="ZhaoGongZuoServlet?id=<%=con2.getId() %>"><%=con2.getTitle()+" 发布日期"+con2.getTime()%></a></td>
</tr>
<%} %>
</table>
查看求购：
...
<%  while(qiougouI.hasNext()){ 
	ErShou con3=(ErShou)qiougouI.next();
%>
<tr>
<td><%=con3.getId() %></td>
<td><a href="ErShouServlet?id=<%=con3.getId() %>"><%=con3.getTitle()+" 发布日期"+con3.getTime() %></a></td>
</tr>
<%} %>
</table>
查看出售：
...
<%  while(chushouI.hasNext()){ 
	ErShouMai con4=(ErShouMai)chushouI.next();
%>
<tr>
<td><%=con4.getId() %></td>
<td><a href="ErShouMaiServlet?id=<%=con4.getId() %>"><%=con4.getTitle()+" 发布日期"+con4.getTime() %></a></td>
</tr>
<%} %>
</table>
查看出租：
...
<%  while(zufangI.hasNext()){ 
	ZuFang con4=(ZuFang)zufangI.next();
%>
<tr>
<td><%=con4.getId() %></td>
<td><a href="ZuFangServlet?id=<%=con4.getId() %>"><%=con4.getTitle()+" 发布日期"+con4.getTime() %></a></td>
</tr>
<%} %>
</table>
<a href="SelectServlet?nowpage=<%=nowpage+1 %>&keyword=<%=keyword %>">下一页</a>
<% 
if(nowpage<=1)
{
%>
<a href="SelectServlet?nowpage=1&keyword=<%=keyword %>">上一页</a>
<%
}else{
%>
<a href="SelectServlet?nowpage=<%=nowpage-1 %>&keyword=<%=keyword %>">上一页</a>
<% }%>
</div>
</div>
