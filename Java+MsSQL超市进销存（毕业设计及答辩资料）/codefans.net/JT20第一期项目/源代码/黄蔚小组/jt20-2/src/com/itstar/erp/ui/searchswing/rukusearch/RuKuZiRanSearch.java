...
public class RuKuZiRanSearch {
	...
	static Map<Integer,String> ywyidnamemap=new HashMap<Integer,String>();      //业务员     编号-----名称
	static{
	...
	static Map<Integer,Integer> prosp =new HashMap<Integer,Integer>();         //商品     商品编号----供应商编号
	static {
	...
	static Map<Integer,String> spidnamemap=new HashMap<Integer,String>();         //供应商     编号----名称
	static {
	...
	public void init(){
		...
		String[] colnames={"入库编号","商品名称","商品进价","入库数量","供应商名称","经手人","入库金额","入库时间"}; 
		...
		try {
			while(rs.next()){
				...
				String stime=new GetTime().format(rukudatetime);                           //入库时间   无----
...
//				{"入库编号","商品名称","商品进价","入库数量","供应商名称","经手人","入库金额","入库时间"}                
				...
				cells[j][6]=String.valueOf(rukujine).trim()+"   元";
