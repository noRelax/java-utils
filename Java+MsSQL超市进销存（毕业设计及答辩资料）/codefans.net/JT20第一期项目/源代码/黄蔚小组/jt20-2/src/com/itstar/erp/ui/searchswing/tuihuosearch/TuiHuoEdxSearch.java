...
public class TuiHuoEdxSearch {
	...
	public void init(){
		...
		Object[][] cells;
		ResultSet rs=new GetRS().getResultSet("select t1.* from tb_tuihuo_info t1,tb_sell_info t2,tb_product_info t3 where t1.sellID=t2.sellID and t2.proID=t3.proID order by t1.thAcount*t3.proPrice desc");
		String[] colnames={"退货金额","销售编号","商品名称","商品进价","销售价格","销售数量","退货数量","退货编号","销售时间","退货时间","退货原因","经手人"}; 
...
//				{"退货编号","销售编号","商品名称","商品进价","销售价格","销售数量","退货数量","退货金额","销售时间","退货时间","退货原因","经手人"};         
//				
				cells[j][0]=String.valueOf(tuihuojine).trim()+"   元";
				...
				cells[j][6]=String.valueOf(tuihuoacount).trim();
		...
		JFrame jf=new JFrame("按退货金额大小排行查询");
