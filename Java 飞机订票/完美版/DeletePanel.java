...
public class DeletePanel extends JPanel  implements ActionListener
{
	...
	//make the radio independence to each other
	...
	//this fnction is waited to be operated 
	public static void updateCityComboBox(String newCity,int flag)
	{
		//the items is to be increased
		if(flag==1)
		{
		...
		try
		{
			...
			if(e.getSource() instanceof JButton)
			{
				...
				else if(radio2.isSelected())
				{
					...
					//make sure this day was one day of the week which the flight has
					for(int i=0;i<=w.length();i++)
					{
						char c=w.charAt(i);
						String s=""+c;
					...
					//if the week of the flight was deleted all,it should be deleted
					if(week.length()==0) 
