...
//public class EmpInfo extends JFrame implements ActionListener{	
	...
	public EmpInfo()
	{
		...
		em = new EmpModel();
		String [] paras = {"1"};
	//	em.query("select * from renshiziliao where 1 = ? ", paras);//where 1 = ?不能写作where1 = ?否则会报错	
		
		em.query("select  userid , username  ,sex, birth, id , xueli ," +
				" married , phone1, phone2 ,youxiang, zhuceshijian, " +
				"beizu  ,zhiwei   from renshiziliao where 1 = ? ", paras);
		count = em.count;
