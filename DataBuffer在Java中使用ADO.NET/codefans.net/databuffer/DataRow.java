/*
 * $Id: DataRow.java,v 1.12.2.4 2006/01/21 05:21:33 david_hall Exp $
 *
...
/**
 * <p>A <CODE>DataRow</CODE> contains a set of values within a {@link DataTable}; the
 * <CODE>DataTable</CODE> defines the {@link DataColumn DataColumns} in each row, and each row
 * always has a value for each <CODE>DataColumn</CODE>. The intersection of a
 * <CODE>DataColumn</CODE> in a <CODE>DataRow</CODE> is called a cell, though cells are not exposed
 * in the <CODE>DataRow's</CODE> API. A <CODE>DataRow</CODE> always belongs to a
 * <CODE>DataTable</CODE>; rows are added to a table using the table's {@link DataTable#appendRow()}
 * or {@link DataTable#appendRowNoEvent()} methods and cannot be instantiated directly. Values in
 * each cell can be changed using the {@link #setValue(String colName, Object value)} or {@link
 * #setValue(DataColumn col, Object value)} methods; values are always <CODE>Objects</CODE>.
 * 
 * <p>A <CODE>DataRow</CODE> always has a status, whose state is relative to the persistence
 * mechanism for the table. Statuses are returned using the row's <CODE>DataRowStatus</CODE>
 * enumeration, through {@link #getStatus()} and can be changed using {@link
 * #setStatus(DataRowStatus)}. Statuses are normally managed without developer
 * intervention. Normally, when rows are loaded from a persistent store, they have a status
 * <CODE>UNCHANGED</CODE>, which indicates that a {@link DataProvider} does not need to synchronize
 * them on the next {@link DataTable#save()} operation. A status of <CODE>INSERTED</CODE> means the
 * row was added to the table and should be added to the persistent store on the next {@link
 * DataTable#save()}, <CODE>UPDATED</CODE> means changes to a row loaded from the persistent store
 * must be written back to the store, and <CODE>DELETED</CODE> means the row should be removed from
 * the store. Adding a row programmatically through the table's {@link DataTable#appendRow()} or
 * {@link DataTable#appendRowNoEvent()} methods gives the row a status of <CODE>INSERTED</CODE>.
 * 
 * <p>Each cell in a row can hold a reference value and a current value. The reference value is
 * normally the value as last read from the persistent store; this value is usually not modified
 * programatically, but may be overridden by a <CODE>DataProvider</CODE> if the row is re-read from
 * the data store. The {@link #setReferenceValue(DataColumn col, Object refValue)} method changes
 * the reference value for a cell.  The current value is the value which is normally changed
 * programmatically or through a user interface. When the current value is different than the
 * reference value, we say the cell has changed. If the row had <CODE>UNCHANGED</CODE> status before
 * the change, its status will change to <CODE>UPDATED</CODE>; this happens either on both the
 * {@link #setValue(DataColumn, Object)} or {@link #setReferenceValue(DataColumn, Object)}
 * methods. An <CODE>INSERTED</CODE> or a <CODE>DELETED</CODE> row can have its cells changed
 * without affecting the row's status.
 * 
 * <p>For performance, comparing the reference to the current value of a cell is done using the Java
 * <CODE>==</CODE> comparison by default, which of course only works for object references that are
 * the same. Identity comparisons are used on all columns if the <CODE>DataTable's</CODE>
 * <CODE>identityComparisonEnabled</CODE> property is set to <CODE>true</CODE>.  If this property is
 * false, one can assign a <CODE>Comparator</CODE> per-column in the table, or per-class (DataColumn
 * type).  If both a per-class and a per-column <CODE>Comparator</CODE> have been assigned, the
 * per-column <CODE>Comparator</CODE> is used.  If the property is false, and no
 * <CODE>Comparator</CODE> is assigned, equality comparison (<CODE>.equals()</CODE>) is used. Note
 * that if you do not assign <CODE>Comparators</CODE>, you may have rows with an
 * <CODE>UPDATED</CODE> status when the value is actually the same, if the <CODE>==</CODE>
 * comparison fails. For accuracy, you should assign a <CODE>Comparator</CODE>, but for efficiency
 * (e.g. columns with large content sizes), you may want a <CODE>Comparator</CODE> that just skips
 * the test altogether.
 * 
 * <p><CODE>DataRows</CODE> support both property change listeners and event listeners; note that
 * row status is a property of the row. Events broadcast {@link RowChangeEvent} messages.
 * @author Richard Bair
...
public class DataRow {
    ...
    public void setReferenceValue(String colName, Object value) {
        setReferenceValue(getColumnFromTable(colName), value);
    }
    ...
    public void setReferenceValue(DataColumn col, Object value) {
        enforceCorrectType(col, value, "ReferenceType");
    ...
    /**
     * Sets the given value to the column with the given name. This will fire
     * a {@link RowChangeEvent} to {@link DataTableListener DataTableListeners} if the value
     ...
     * comments.
     * @return this DataRow, allowing stacked setValue calls
     */
    ...
    /**
     * Sets the given value to the specified DataColumn with the given name. This will fire
     * a {@link RowChangeEvent} to {@link DataTableListener DataTableListeners} if the value
     * changed; the RCE will be fired after the status change, so you can check for 
     * status changes there. If the status changes, a property change event will also 
     * be broadcast, for the status property.
    ...
    public DataRow setValue(DataColumn col, Object value) {
        enforceCorrectType(col, value, "value");

        Object oldValue = getValue(col);
        DataCell cell = getCell(col);
        cell.setValue(table, col, value); 
        ...
        // NOTE: fire row change event after status change so listeners can pick up status
    ...
    /**
     * Resets all columns in the row to have their current value be their reference value,
     * effectively reverting the effects of calling {@link #setValue(DataColumn, Object)} till
     * now. After this method call, the row will once again regain its normal status (INSERTED or
     * UNCHANGED). For a discussion on reference values, see the class docs.
     */
    ...
    /**
     * Resets the named column in the row to its reference value, effectively reverting the effects
     * of calling {@link #setValue(DataColumn, Object)} till now. After this method call, the row
     * may once again regain its normal status (INSERTED or UNCHANGED) if there are no longer any
    ...
    public void resetToReferenceValue(String colName) {
        resetToReferenceValue(getColumnFromTable(colName));
    }
    
    /**
     * Resets the DataColumn in the row to its reference value, effectively reverting the effects of
     * calling {@link #setValue(DataColumn, Object)} till now. After this method call, the row may
     * once again regain its normal status (INSERTED or UNCHANGED) if there are no longer any
     * modified columns. For a discussion on reference values, see the class docs.
    ...
    public Object getReferenceValue(String colName) {
        return getReferenceValue(getColumnFromTable(colName));
    }
    ...
    /**
     * Returns the current value for the column.  This is a callback method
     * called by the column if it is unable to determine the value.  Computed
     * Colu
     * @return the value for the given DataColumn.
     * @param col The DataColumn for which to return the current value.
     */
    protected Object getValueByColumn(DataColumn col) {
    ...
    /**
     * Notifies the row that it has been dropped from its table.
     */
     ...
     */
    private void enforceCorrectType(DataColumn col, Object val, String propName) {
        // Ensure that the column exists
        if (col == null) 
            throw new IllegalArgumentException("A DataColumn is required");

        // Ensure that the value is correct
        if (val != null && !col.getType().isInstance(val)) {
            String msg = "Column \"{0}\" type {1}: cannot set {2} to {3}";
            Object[] args = new Object[]{ col.getName(), col.getType().getName(), propName, val};
     ...
     * @throws java.lang.IllegalArgumentException if no such column exists
     */
    private DataColumn getColumnFromTable(String colName) {
        ...
        Object[] args = new Object[]{colName, table.getName()};
        throw new IllegalArgumentException(MessageFormat.format(msg, args));
    }
    ...
    private static final class DataCell {
         ...
         * @throw IllegalArgumentException if the value is not of the correct type
         * for the cell
         */
        ...
        /**
         * Returns true if the two values are the same, using either <CODE>==</CODE>, or a
         * <CODE>Comparator</CODE>; see class docs on DataRow.
         * @param table The DataTable for the row.
         * @param col The DataColumn for the cell.
